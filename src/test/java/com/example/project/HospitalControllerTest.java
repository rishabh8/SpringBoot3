...
public class HospitalControllerTest {
	...
	public void retrievetest_ok() throws Exception {
		addhospital_ok();
		mockMvc.perform(get("/test/hospitals/1000")).andDo(print()).andExpect(status().isOk())
	...
	public void addhospital_ok() throws Exception {
		...
		mockMvc.perform(post("/test/hospitals/")// .andDo(print())
				.content(hospJson).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
		mockMvc.perform(post("/test/hospitals/")// .andDo(print())
				.content(hospJson1).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	}
	...
	public void updatehospital_ok() throws Exception {
		...
		mockMvc.perform(get("/test/hospitals/1001")).andDo(print()).andExpect(status().isOk())
